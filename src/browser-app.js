...
const elements = {
  ...
  academySearchForm: document.querySelector('#academy-search-form'),
  academySearchInput: document.querySelector('#academy-search-input'),
  academySearchResults: document.querySelector('#academy-search-results'),
  ...
  lessonAudio: document.querySelector('#lesson-audio'),
  lessonAudioToggle: document.querySelector('#lesson-audio-toggle'),
  lessonAudioStop: document.querySelector('#lesson-audio-stop'),
  lessonAudioRate: document.querySelector('#lesson-audio-rate'),
...
window.addEventListener('error', event => {
  reportClientError('WINDOW_ERROR', event.error?.message || event.message).catch(() => {});
});
window.addEventListener('unhandledrejection', event => {
  reportClientError('UNHANDLED_REJECTION', event.reason?.message || String(event.reason || 'Unhandled promise rejection')).catch(() => {});
});

async function reportClientError(code, summary) {
  const safeSummary = String(summary || 'Client error').replace(/https?:\/\/\S+/g, '[url]').slice(0, 500);
  await fetch('/api/client-error', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ code, summary: safeSummary, path: window.location.pathname }),
    keepalive: true,
  });
}
...
async function initialize() {
  ...
  await initializeCloud({ eager: shouldLoadMemberCloud() });
...
function shouldLoadMemberCloud() {
  const params = new URLSearchParams(window.location.search);
  return /^#app-/.test(window.location.hash)
...
async function ensureCloudLoaded({ restoreSession = true } = {}) {
  ...
  const cloudModuleUrl = '/cloud.js?v=0.36.10';
  state.cloudLoading = import(cloudModuleUrl)
    ...
    .finally(() => { state.cloudLoading = null; });
  return state.cloudLoading;
}

async function initializeCloud({ eager = false } = {}) {
  try {
...
function bindEvents() {
  ...
  elements.academySearchForm?.addEventListener('submit', searchAcademy);
  elements.academySearchResults?.addEventListener('click', event => {
    const button = event.target.closest('[data-search-course]');
    if (button) openCourse(button.dataset.searchCourse, button.dataset.searchItem);
  });
  ...
  elements.lessonAudioStop?.addEventListener('click', stopLessonAudio);
  elements.lessonAudioRate?.addEventListener('change', () => {
    if (state.lessonAudio.active || state.lessonAudio.paused) stopLessonAudio();
    elements.lessonAudioStatus.textContent = `Rychlost ${elements.lessonAudioRate.value.replace('.', ',')}× je připravená.`;
  });
...
function switchView(view) {
  ...
  document.querySelectorAll('[data-view]').forEach(button => {
    ...
    const navigationControl = button.classList.contains('nav-item') || Boolean(button.closest('.mobile-nav'));
    if (active && navigationControl) button.setAttribute('aria-current', 'page');
    else button.removeAttribute('aria-current');
  });
  if (state.currentView !== 'academy') stopLessonAudio({ quiet: true });
...
async function searchAcademy(event) {
  event.preventDefault();
  const query = elements.academySearchInput.value.trim();
  if (query.length < 2) return;
  elements.academySearchResults.hidden = false;
  elements.academySearchResults.innerHTML = '<p class="academy-search-loading">Prohledávám celou Academy…</p>';
  try {
    const payload = await authenticatedRequest(`/api/course-search?q=${encodeURIComponent(query)}`, { cache: 'no-store' });
    state.academySearchResults = Array.isArray(payload.items) ? payload.items : [];
    if (!state.academySearchResults.length) {
      elements.academySearchResults.innerHTML = `<div class="academy-search-empty"><strong>Nic přesného jsem nenašla.</strong><p>Zkus kratší výraz nebo jiné slovo.</p></div>`;
      return;
    }
    elements.academySearchResults.innerHTML = `<header><strong>${state.academySearchResults.length} nalezených částí</strong><button type="button" data-clear-academy-search>Zavřít výsledky</button></header>${state.academySearchResults.map(result => `
      <button type="button" class="academy-search-result" data-search-course="${escapeHtml(result.courseSlug)}" data-search-item="${escapeHtml(result.itemId)}">
        <span>${escapeHtml(result.courseTitle)} · ${escapeHtml(result.moduleTitle)}</span>
        <strong>${escapeHtml(result.itemTitle)}</strong>
        <p>${escapeHtml(result.snippet)}</p>
        <small>${result.minutes || 0} min · Otevřít přesnou část →</small>
      </button>`).join('')}`;
    elements.academySearchResults.querySelector('[data-clear-academy-search]')?.addEventListener('click', () => {
      elements.academySearchResults.hidden = true;
      elements.academySearchResults.innerHTML = '';
      state.academySearchResults = [];
    });
  } catch (error) {
    elements.academySearchResults.innerHTML = `<div class="academy-search-empty"><strong>Hledání se nepodařilo.</strong><p>${escapeHtml(error.message)}</p></div>`;
  }
}

async function openCourse(slug, itemId = '') {
  switchView('academy');
  try {
    ...
    elements.masteryToggle.setAttribute('aria-expanded', 'false');
    renderCourseMastery();
    const items = flattenCourseItems(state.activeCourse);
    const requestedItem = itemId ? items.findIndex(item => item.id === itemId) : -1;
    const firstIncomplete = items.findIndex(item => !state.courseProgress.has(progressKey(state.activeCourse, item)));
    state.activeItemIndex = requestedItem >= 0 ? requestedItem : firstIncomplete >= 0 ? firstIncomplete : 0;
...
function prepareLessonAudio(item) {
  if (!elements.lessonAudioToggle) return;
  const supported = 'speechSynthesis' in window && 'SpeechSynthesisUtterance' in window;
  const rawText = [item?.title, elements.lessonContent?.textContent]
    .filter(Boolean)
    .join('. ')
    .replace(/\s+/g, ' ')
    .trim();
  state.lessonAudio = {
    chunks: splitLessonAudioText(rawText),
    index: 0,
    active: false,
    paused: false,
    utterance: null,
  };
  elements.lessonAudioToggle.disabled = !supported || !state.lessonAudio.chunks.length;
  elements.lessonAudioStop.disabled = true;
  elements.lessonAudioToggle.setAttribute('aria-pressed', 'false');
  elements.lessonAudioToggle.textContent = '▶ Poslechnout';
  elements.lessonAudioStatus.textContent = supported
    ? `Připraveno přibližně ${Math.max(1, Math.round(rawText.split(/\s+/).length / 145))} min poslechu. Automatický hlas zůstává v tomto zařízení.`
    : 'Tento prohlížeč automatický poslech nepodporuje. Text lekce zůstává plně dostupný.';
}

function splitLessonAudioText(text) {
  if (!text) return [];
  ...
  sentences.forEach(sentence => {
    ...
    if (!clean) return;
    if (`${current} ${clean}`.trim().length > 230 && current) {
...
function toggleLessonAudio() {
  if (!('speechSynthesis' in window) || !state.lessonAudio.chunks.length) return;
  if (state.lessonAudio.active && !state.lessonAudio.paused) {
    window.speechSynthesis.pause();
    state.lessonAudio.paused = true;
    elements.lessonAudioToggle.textContent = '▶ Pokračovat';
    elements.lessonAudioToggle.setAttribute('aria-pressed', 'false');
    elements.lessonAudioStatus.textContent = 'Poslech je pozastavený.';
    return;
  }
  if (state.lessonAudio.active && state.lessonAudio.paused) {
    window.speechSynthesis.resume();
    state.lessonAudio.paused = false;
    elements.lessonAudioToggle.textContent = 'Ⅱ Pauza';
    elements.lessonAudioToggle.setAttribute('aria-pressed', 'true');
    elements.lessonAudioStatus.textContent = `Přehrávám část ${state.lessonAudio.index + 1} z ${state.lessonAudio.chunks.length}.`;
    return;
  }
  state.lessonAudio.active = true;
  state.lessonAudio.paused = false;
  state.lessonAudio.index = 0;
  elements.lessonAudioStop.disabled = false;
  elements.lessonAudioToggle.textContent = 'Ⅱ Pauza';
  elements.lessonAudioToggle.setAttribute('aria-pressed', 'true');
  speakNextLessonAudioChunk();
}

function speakNextLessonAudioChunk() {
  if (!state.lessonAudio.active || state.lessonAudio.paused) return;
  if (state.lessonAudio.index >= state.lessonAudio.chunks.length) {
    stopLessonAudio({ quiet: true });
    elements.lessonAudioStatus.textContent = 'Poslech této části je dokončený.';
    return;
  }
  const utterance = new SpeechSynthesisUtterance(state.lessonAudio.chunks[state.lessonAudio.index]);
  utterance.lang = 'cs-CZ';
  utterance.rate = Number(elements.lessonAudioRate?.value || 1);
  const voices = window.speechSynthesis.getVoices();
  utterance.voice = voices.find(voice => voice.lang?.toLowerCase().startsWith('cs')) || null;
  utterance.onend = () => {
    if (!state.lessonAudio.active) return;
    state.lessonAudio.index += 1;
    speakNextLessonAudioChunk();
  };
  utterance.onerror = event => {
    if (event.error === 'canceled' || event.error === 'interrupted') return;
    stopLessonAudio({ quiet: true });
    elements.lessonAudioStatus.textContent = 'Poslech se v tomto prohlížeči přerušil. Můžeš ho spustit znovu.';
  };
  state.lessonAudio.utterance = utterance;
  elements.lessonAudioStatus.textContent = `Přehrávám část ${state.lessonAudio.index + 1} z ${state.lessonAudio.chunks.length}.`;
  window.speechSynthesis.speak(utterance);
}

function stopLessonAudio({ quiet = false } = {}) {
  if ('speechSynthesis' in window) window.speechSynthesis.cancel();
  state.lessonAudio.active = false;
  state.lessonAudio.paused = false;
  state.lessonAudio.index = 0;
  state.lessonAudio.utterance = null;
  if (!elements.lessonAudioToggle) return;
  elements.lessonAudioToggle.textContent = '▶ Poslechnout';
  elements.lessonAudioToggle.setAttribute('aria-pressed', 'false');
  elements.lessonAudioStop.disabled = true;
  if (!quiet) elements.lessonAudioStatus.textContent = 'Poslech byl zastavený.';
}
...
function renderCourseOutline() {
  ...
  elements.courseOutline.innerHTML = state.activeCourse.modules.map((module, moduleIndex) => {
    const moduleItems = module.items.map(item => {
      ...
      const completed = state.courseProgress.has(progressKey(state.activeCourse, item));
      const active = index === state.activeItemIndex;
      return `<button type="button" class="outline-item ${active ? 'active' : ''} ${completed ? 'completed' : ''}" data-course-item="${index}" ${active ? 'aria-current="step"' : ''}><span>${completed ? '✓' : String(index + 1).padStart(2, '0')}</span><b>${escapeHtml(item.title)}</b><small>${courseKindLabel(item.kind, state.activeCourse)}</small></button>`;
    }).join('');
