...
const coursePath = join(ROOT, 'data', 'course-strategicka-partnerstvi-business-development.md');
const materialPath = join(ROOT, 'data', 'course-strategicka-partnerstvi-business-development-materials.json');
const audioPath = join(ROOT, 'data', 'course-strategicka-partnerstvi-business-development-audio-scripts.md');
...
const audioPack = materials.find(material => material.id === 'bd-audio-pack');
audioPack.resourceMarkdown = audioScripts;
attachCourseMastery(course);

test('Business Development výcvik má reálný 28hodinový rozsah a 84 částí', () => {
  assert.equal(course.id, 'strategicka-partnerstvi-business-development');
  ...
  assert.match(course.certificate.note, /negarantuje uzavření dohody/i);
});

test('každý BD modul obsahuje aktivní studium, laboratoř, partnerskou simulaci a test', () => {
...
test('BD výcvik má 28 veřejných materiálů a deset přesných audio briefingů', () => {
...
test('BD lektorka má oddělenou roli a 84 realistických partnerských simulací', () => {
  assert.equal(course.trainer.label, 'Lektorka strategických partnerství a dealová oponentka');
  assert.equal(course.trainer.studyAction, 'Pracovat s BD lektorkou');
  assert.match(course.trainer.studyOpening, /fakta od partnerských hypotéz/i);
  assert.match(course.trainer.description, /právní či datovou oponentku/i);
