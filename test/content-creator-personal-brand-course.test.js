...
const coursePath = join(ROOT, 'data', 'course-content-creator-personal-brand-studio.md');
const materialPath = join(ROOT, 'data', 'course-content-creator-personal-brand-studio-materials.json');
const audioPath = join(ROOT, 'data', 'course-content-creator-personal-brand-studio-audio-scripts.md');
...
materials.find(material => material.id === 'ccp-audio-pack').resourceMarkdown = audioScripts;
attachCourseMastery(course);

test('Content Creator & Personal Brand Studio má 18 modulů, 108 částí a 36 hodin', () => {
  assert.equal(course.id, 'content-creator-personal-brand-studio');
  ...
  assert.ok(words >= 6500, `výcvik má pouze ${words} slov`);
  assert.equal(course.durationHours, 36);
  assert.equal(course.categoryId, COURSE_CATEGORIES.MARKETING.id);
  assert.match(course.certificate.note, /negarantuje viralitu, dosah, retenci, počet sledujících, partnerství, příjem/i);
});

test('každý modul má aktivní výklad, laboratoř, creator review a test i úplnou knowledge coverage', () => {
...
test('kurz má vlastní content creator lektorku a 108 production review simulací', () => {
  assert.equal(course.trainer.label, 'Lektorka content creation a personal brand production review');
  assert.equal(course.trainer.studyAction, 'Pracovat s content creator lektorkou');
  assert.match(course.trainer.studyOpening, /lidskou hodnotu od výkonu účtu/i);
