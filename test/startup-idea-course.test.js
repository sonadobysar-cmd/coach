...
const coursePath = join(ROOT, 'data', 'course-napad-k-overene-prilezitosti.md');
const materialPath = join(ROOT, 'data', 'course-napad-k-overene-prilezitosti-materials.json');
const audioPath = join(ROOT, 'data', 'course-napad-k-overene-prilezitosti-audio-scripts.md');
...
const audioPack = materials.find(material => material.id === 'idea-audio-pack');
audioPack.resourceMarkdown = audioScripts;
attachCourseMastery(course);

test('startup validation výcvik má reálný 28hodinový rozsah a 84 částí', () => {
  assert.equal(course.id, 'napad-k-overene-prilezitosti');
  ...
  assert.match(course.certificate.note, /negarantuje životaschopnost nápadu/i);
});

test('každý validační modul obsahuje aktivní studium, laboratoř, simulaci a test', () => {
...
test('validační lektorka má oddělenou roli a 84 důkazně poctivých simulací', () => {
  assert.equal(course.trainer.label, 'Validační lektorka a startupová oponentka');
  assert.equal(course.trainer.studyAction, 'Pracovat s validační lektorkou');
  assert.match(course.trainer.studyOpening, /AI návrhu/i);
