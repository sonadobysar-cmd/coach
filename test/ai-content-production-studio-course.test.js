...
const coursePath = join(ROOT, 'data', 'course-ai-content-production-studio.md');
const materialPath = join(ROOT, 'data', 'course-ai-content-production-studio-materials.json');
const audioPath = join(ROOT, 'data', 'course-ai-content-production-studio-audio-scripts.md');
...
materials.find(material => material.id === 'aic-audio-pack').resourceMarkdown = audioScripts;
attachCourseMastery(course);

test('AI Content Production Studio má 12 modulů, 72 částí a 24 hodin', () => {
  assert.equal(course.id, 'ai-content-production-studio');
  ...
  assert.ok(words >= 3200, `výcvik má pouze ${words} slov`);
  assert.equal(course.durationHours, 24);
  assert.equal(course.categoryId, COURSE_CATEGORIES.MARKETING.id);
  assert.match(course.certificate.note, /negarantuje viralitu, ranking, úsporu, dosah, konverze, prodej/i);
});
...
test('každý modul má aktivní výklad, laboratoř, simulaci a test i úplnou knowledge coverage', () => {
  ...
  assert.equal(items.filter(item => item.kind === 'lesson').length, 36);
...
test('kurz má 24 veřejných materiálů a 10 přesných audio vedení', () => {
  assert.equal(materials.length, 25);
  const publicMaterials = publicCourseDetail(course).materials;
  assert.equal(publicMaterials.length, 24);
...
test('kurz má vlastní AI content lektorku a 72 produkčních simulací', () => {
  assert.equal(course.trainer.label, 'Lektorka AI content produkce a multimodálního workflow');
  assert.equal(course.trainer.studyAction, 'Pracovat s AI content lektorkou');
  assert.match(course.trainer.studyOpening, /zdroj, vlastní lidský vklad, AI draft/i);
