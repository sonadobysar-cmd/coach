...
const out=new URL('../reports/bench-training.json',import.meta.url);
...
try {
 for(const compact of ['0','1']) {
  ...
  for(const [activity,phase,messages] of [
   ['study','study',[{role:'user',content:'Vysvětli mi hlavní princip lekce a dej jeden příklad.'}]],
   ['simulation','roleplay',[{role:'user',content:'Co by sis přála na konci dnešního rozhovoru ujasnit?'}]],
   ['simulation','roleplay',[...long,{role:'user',content:'Který krok tedy chceš zkusit jako první?'}]],
   ['simulation','debrief',[...long,{role:'user',content:'Ukončeme nácvik a vyhodnoť moje vedení rozhovoru.'}]],
  ]) {
   const before=events.length;
   const r=await aiMeterContext.run(context,()=>auditTraining({messages,course,item,activity,phase,memory:{},difficulty:'standard'}));
   results.push({compact,role:activity,phase,text:r.text,quality:r.qualityGate,provider:r.provider,events:events.slice(before)});
